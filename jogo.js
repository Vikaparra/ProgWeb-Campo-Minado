...
function gravarDados(){
    ...
    if(!ajax){
                    ...
                    if(ajax.status === 200) {
                        let resposta = JSON.parse(ajax.responseText);
                            if(resposta.resposta == 1){
                                console.log("Partida gravada com sucesso")
                            }else{
                                console.log("Erro ao gravar a partida");
                            }
                    }else{
        ...
        ajax.send('campo_nome=' + encodeURIComponent(campo_nome) + '&quantity=' + encodeURIComponent(quantity) + '&myRange=' + encodeURIComponent(myRange) + '&modo_jogo=' + encodeURIComponent(modo_jogo) + '&time=' + encodeURIComponent(time) + '&resultado=' + encodeURIComponent(resultado) + '&data_hora=' + encodeURIComponent(data_hora));
        }
}
